...
export function inspect(buffer) {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    const checksumPresent = !zero(bytes.subarray(checksumOffset, dataOffset));
    ...
    slots.push({ index: i + 1, active, name, level, seconds, version,
      checksumPresent, checksum: hex(bytes.subarray(checksumOffset, dataOffset)), issues });
  }
  return { fileBytes: bytes.length, accountVersion, steamId, slots, warnings,
